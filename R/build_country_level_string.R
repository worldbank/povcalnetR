...
build_country_level_string <- function(country,
                                       ...
                                       format = "json") {
  # CHECK inputs
  assertthat::assert_that(length(unique(purrr::map_int(list(country, poverty_line, year), length))) == 1,
                          msg = "When parameter_specification = 'country_level',
                          please ensure that you submit the same number of:
                          country; poverty_line; and year")
  ...
  if (!is.null(ppp)) {
    ...
    out <- paste(country, poverty_line, year, ppp, collapse = "&")
    out <- stringr::str_replace_all(out, pattern = " ", replacement = "&")
  } else {
    out <- paste(country, poverty_line, year, collapse = "&")
    out <- stringr::str_replace_all(out, pattern = " ", replacement = "&")
  }
